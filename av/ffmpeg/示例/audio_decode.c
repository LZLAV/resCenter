//// 解码之后将生成的 audio stream 放到 queue中 /////
//// 展晓凯项目：采用链表，出队则删除该节点，入队生成新节点 //// 
...
int AccompanyDecoder::init(const char* audioFile) {
	...
	avFormatContext->max_analyze_duration = 50000;		//分析时长，50ms数据
	//检查在文件中的流的信息
	...
	if (audioStream->time_base.den && audioStream->time_base.num)		//时间戳base(刻度)
