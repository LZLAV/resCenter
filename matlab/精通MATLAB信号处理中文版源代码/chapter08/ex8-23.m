load tire
[ca1,ch1,cv1,cd1]=dwt2(X,'sym4'); 	%使用sym4小波对信号进行一层小波分解
codca1=wcodemat(ca1,192);
codch1=wcodemat(ch1,192);
codcv1=wcodemat(cv1,192);
codcd1=wcodemat(cd1,192);
codx=[codca1,codch1,codcv1,codcd1] 	%将四个系数图像组合为一个图像
rca1=ca1; 	%复制原图像的小波系数
rch1=ch1;
rcv1=cv1;
rcd1=cd1;
rch1(33:97,33:97)=zeros(65,65); 	%将三个细节系数的中部置零
...
codrx=[codrca1,codrch1,codrcv1,codrcd1] 	%将处理后的系数图像组合为一个图像
rx=idwt2(rca1,rch1,rcv1,rcd1,'sym4'); 	%重建处理后的系数
subplot(221);
...
colormap(map);
title('处理后各层系数图像');
per=norm(rx)/norm(X) 	%求压缩信号的能量成分
per =1.0000
err=norm(rx-X) 	%求压缩信号与原信号的标准差
